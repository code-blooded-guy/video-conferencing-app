...
const welcomeImg = 'https://livestar.co.il/wp-content/uploads/2021/02/Favicon.svg';
const shareUrlImg = 'https://livestar.co.il/wp-content/uploads/2021/02/Favicon.svg';
const leaveRoomImg = '../images/leave-room.png';
const confirmImg = 'https://livestar.co.il/wp-content/uploads/2021/02/Favicon.svg';
const fileSharingImg = 'https://livestar.co.il/wp-content/uploads/2021/02/Favicon.svg';
...
function getHtmlElementsById() {
    ...
    participants = getId('participants');
}
...
function initClientPeer() {
    ...
    signalingSocket.on('peerRole', handlePeerName);
...
function setButtonsBarPosition(position) {
    ...
    if (isMobileDevice) {
        ...
        // document.documentElement.style.setProperty('--btns-left', '53%');
        // document.documentElement.style.setProperty('--btns-margin-left', '-300px');
        // document.documentElement.style.setProperty('--btns-width', '21em');
        // // document.documentElement.style.setProperty('white-space', 'nowrap');
...
function loadRemoteMediaStream(stream, peers, peer_id) {
    ...
    console.log('peers[peer_id]', peers[peer_id])
    ...
    let peer_role = peers[peer_id]['peer_role'];
    ...
    remoteVideoParagraph.className = 'videoPeerName mr-58';
    ...
    remoteVideoAvatarImage.style.width = '50%'
    ...
    console.log('peer_role', peer_role)
    if (peer_role === 'host') {
        if(isMobileDevice){
            remoteVideoWrap.style.height = '37.5vh';
        }
        
        remoteVideoWrap.className = 'video one';
...
function setUserBtn() {
    ...
    // userTheme.addEventListener('click', (e) => {
    ...
    //         e.target.className = 'fas fa-undo';
    //         document.documentElement.style.setProperty('--msger-bg', 'rgba(0, 0, 0, 0.100)');
    //         document.documentElement.style.setProperty('--box-shadow', 'rgba(0, 0, 0, 0.100)');
    //         usersDraggable.style.boxShadow = ''
    //     } else {
    //         e.target.className = 'fas fa-ghost';
    //         document.documentElement.style.setProperty('--msger-bg', 'linear-gradient(to left, #000000, #383838)');
    //         document.documentElement.style.setProperty('--box-shadow', '3px 3px 6px #0500ff, -3px -3px 6px #da05f3');
    //     }
    // });
...
function showButtonsBarAndMenu() {
    if (isButtonsVisible || (isMobileDevice && isChatRoomVisible) || (isMobileDevice && isUserVisible) || (isMobileDevice && isMySettingsVisible)) return
    if (isMobileDevice) {
        buttonsBar.style['overflow-x'] = 'auto';
        buttonsBar.style['overflow-y'] = 'hidden';
        buttonsBar.style.width = '21em';
        buttonsBar.style['white-space'] = 'nowrap';
        buttonsBar.style.top = '95%';
        buttonsBar.style.display = 'block';
        participants.style['margin-left'] = '-31px';
        participants.style['margin-top'] = '0px';
        participants.style['margin-bottom'] = '0px';
        participants.style.display = 'block'
        usersDraggable.style.display = 'block'
        usersDraggable.style.left = '50%';
        usersDraggable.style.top = '48%';
        usersDraggable.style.height = '19%';
        usersDraggable.style.width = '95%';
        usersDraggable.style['max-width'] = 'none';
        usersDraggable.style['--msger-bg'] = 'trasparent';
        usersDraggable.style['overflow-x'] = 'auto';
        usersDraggable.style['overflow-y'] = 'hidden';
        usersDraggable.style['white-space'] = 'nowrap';
        
        msgerDraggable.style.display = 'flex'
        msgerDraggable.style.left = '52%';
        msgerDraggable.style.top = '74%';
        msgerDraggable.style.height = '33%';
        msgerDraggable.style.width = '22em';
        msgerDraggable.style['max-width'] = 'none';
...
async function shareRoomUrl() {
    let myRoomUrl = (window.location.href).toString();
    var shareUrl = fakeReplace(myRoomUrl,'host','user');
    // navigator share
...
function makeRoomQR() {
    let qr = new QRious({
        ...
        value: window.location.href.replace('host', 'user'),
    });
    qr.set({
        size: 256,
    });
...
function copyRoomURL() {
    let myRoomUrl = (window.location.href).toString();
    let roomURL = fakeReplace(myRoomUrl,'host','user');
...
/**
 * Set the user on full screen mode for mobile
 */
function setUserForMobile() {
    if (isMobileDevice) {
        document.documentElement.style.setProperty('--msger-height', '99%');
        document.documentElement.style.setProperty('--msger-width', '99%');
    } else {
        // make chat room draggable for desktop
        // dragElement(usersDraggable, userHeader);
    }
}
...
function showChatRoomDraggable() {
    playSound('newMessage');
    if (isMobileDevice) {
        buttonsBar.style.display = 'bolck';
        isButtonsVisible = false;
    }
    chatRoomBtn.className = 'fas fa-comment-slash';
    // msgerDraggable.style.width = '28.5%';
    // msgerDraggable.style.left = '89%';
    // msgerDraggable.style.height = '100%';
    // msgerDraggable.style.top = '50%';
    // myVideoWrap.style.width = '88vw';
    msgerDraggable.style.display = 'flex';
...
function showUserDraggable() {
    // playSound('newMessage');
    if (isMobileDevice) {
        buttonsBar.style.display = 'block';
        isButtonsVisible = false;
    }
    // userBtn.className = 'fas fa-user-slash';
    // msgerDraggable.style.top = '0px';
    // msgerDraggable.style.float = 'left';
    // msgerDraggable.style.height = '100%';
    // msgerDraggable.style.width = '21%';
    // msgerDraggable.style.display = 'flex';
    isUserVisible = true;
    // only for desktop
    // if (!isMobileDevice) {
 ...
 */
function hideUsers() {
    usersDraggable.style.display = 'none';
    // msgerEmojiPicker.style.display = 'none';
    // userBtn.className = 'fas fa-users';
    isUserVisible = false;
    // only for desktop
    // if (!isMobileDevice) {
    //     tippy(userBtn, {
    //         content: 'OPEN all participants',
    //         placement: 'right-start',
    //     });
    // }
}
...
function handlePeerName(config) {
    ...
    if (msgerPeerName) {
        msgerPeerName.innerHTML = `&nbsp;${peer_name} (${peer_role})`;
...
function setPeerVideoStatus(peer_id, status) {
    ...
    peerVideoAvatarImage.style.width = '50%';
